...
{
    return [[self alloc] initWithWebView:aWebView andMatches:someMatches];
}
...
- (id)initWithWebView:(WebView *)aWebView andMatches:(NSArray *)someMatches
{
    ...
    if(scrollView && scrollView.verticalScroller && scrollView.verticalScroller.frame.origin.x > 0 && !scrollView.verticalScroller.isHidden)
    {
        ...
        if (!(self = [self initWithFrame:NSMakeRect(scrollerFrame.origin.x+knobRect.origin.x, scrollerFrame.origin.y+horizontalRect.size.height, knobRect.size.width, scrollerFrame.size.height)])) return nil;
        if(self)
        {
